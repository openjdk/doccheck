 ...
 */
class HtmlFileChecker implements FileChecker {
    private final CharsetDecoder decoder = Charset.forName("UTF-8").newDecoder()
            .onMalformedInput(CodingErrorAction.IGNORE)
            .onUnmappableCharacter(CodingErrorAction.IGNORE);

    private final Log log;
    private final List<HtmlChecker> htmlCheckers;

    HtmlFileChecker(Log log, List<HtmlChecker> htmlCheckers) {
        this.log = log;
        this.htmlCheckers = htmlCheckers;
    }
    ...
    public void report(Reporter reporter) {
        htmlCheckers.forEach(c -> c.report(reporter));
    }
    ...
    public void close() throws IOException {
        ...
        for (HtmlChecker c : htmlCheckers) {
            try {
                c.close();
            } catch (IOException e) {
                if (t == null) {
    ...
    private void read(Path path) {
        try (BufferedReader r = new BufferedReader(
            ...
            log.error(path, lineNumber, t);
            log.printStackTrace(t);
        }
    }

    private void startFile(Path path) {
        htmlCheckers.forEach(c -> c.startFile(path));
    }

    private void endFile() {
        htmlCheckers.forEach(c -> c.endFile());
    }

    private void docType(String s) {
        htmlCheckers.forEach(c -> c.docType(lineNumber, s));
    }

    private void startElement(String name, Map<String,String> attrs, boolean selfClosing) {
        htmlCheckers.forEach(c -> c.startElement(lineNumber, name, attrs, selfClosing));
    }
    ...
    protected void html() throws IOException {
        ...
        if (!inScript) {
            log.error(path, lineNumber, "bad html");
        }
    }
